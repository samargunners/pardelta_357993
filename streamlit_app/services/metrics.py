...
def get_topline(pc_number: str, start: date, end: date) -> dict:
    ...
    sales_df = fetch_df(
        ...
        filters=[lambda q: q.eq("pc_number", pc_number),
...
def get_hourly_sales(pc_number: str, day: date) -> pd.DataFrame:
    ...
    # Optional: aggregate to hour in case there are multiple product rows per time
    out = df.groupby("time", as_index=False).agg({"value":"sum","quantity":"sum"}).sort_values("time")
    return out

def get_waste_summary(pc_number: str, start: date, end: date) -> pd.DataFrame:
    df = fetch_df(
        T_WASTE,
        select="date,ordered_qty,wasted_qty,waste_percent,waste_dollar,expected_consumption,product_type,pc_number,store_name",
        filters=[lambda q: q.eq("pc_number", pc_number),
                 lambda q: _between(q, "date", start, end)]
    )
    return df
